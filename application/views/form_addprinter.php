						...
						<div class="panel-body">
							<form class="form-horizontal form-bordered" method="post" action="<?php echo site_url('proses_inventory/insert_printer');?>">
								...
								<div class="form-group">
									<?php
				                      $qpc=$this->db->query("SELECT * FROM inventory_printer")->num_rows();
				                      if($qpc == 0){$x=1;}
				                      elseif($qpc >= 1)
				                      {
				                        $fore=$this->db->query("SELECT * FROM inventory_printer")->result();//harus ada nilai
				                        foreach ($fore as $key){}
									...
									<div class="col-md-6">
										<input type="text" name="kode" class="form-control input-sm " id="inputDefault" value="IT/PRINTER/<?php echo $x; ?>" readonly="" >
									</div>
								</div>
